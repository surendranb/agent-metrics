	...
	public static function probe() {
		...
		foreach ( $candidates as $label => $dir ) {
			if ( is_file( $dir ) ) {
				...
				$diag[] = array( 'label' => $label, 'path' => $dir, 'status' => 'not_found' );
				continue;
			}
			if ( ! $files ) {
				$diag[] = array( 'label' => $label, 'path' => $dir, 'status' => 'not_found' );
			...
			foreach ( $files as $f ) {
				...
				$diag[] = array( 'label' => $label, 'path' => $f, 'status' => 'ok' );
				return array( 'path' => $f, 'error' => null, 'diagnostics' => $diag );
			}
			$diag[] = array( 'label' => $label, 'path' => $dir, 'status' => 'unreadable' );
		}
		return array( 'path' => false, 'error' => 'no readable log file found', 'diagnostics' => $diag );
	}
