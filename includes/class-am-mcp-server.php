...
class AM_MCP_Server {
	...
	private static function dispatch( $method, $params ) {
		switch ( $method ) {
			...
			default:
				throw new Exception( 'Method not found: ' . esc_html( $method ) );
		}
	}
			...
			array(
				...
				'inputSchema' => array(
					...
					'properties' => array(
						'top_n' => array(
							'type'        => 'number',
							'description' => 'Return only the top N bots (default 10).',
						),
					),
				),
			),
			array(
				...
				'inputSchema' => array(
					...
					'properties' => array(
						'bot' => array(
							'type'        => 'string',
							'description' => 'Optional bot slug to filter (e.g. gptbot).',
						),
					),
				),
			),
	...
	private static function call_tool( $params ) {
		...
		switch ( $name ) {
			...
			case 'bot_summary':
				...
				usort(
					...
					$text = array(
						'total_hits'  => $total,
						'unique_bots' => count( $rows ),
						'bots'        => array_slice( $rows, 0, $top_n ),
					);
				break;
			case 'bot_breakdown':
		...
		return array(
			'content' => array(
				array(
					'type' => 'text',
					'text' => wp_json_encode( $text, JSON_PRETTY_PRINT ),
				),
			),
		);
	}
		...
		return array(
			...
			array(
				...
				'arguments'   => array(
					array(
						'name'        => 'bot',
						'description' => 'Optional bot slug to focus on (e.g. gptbot).',
						'required'    => false,
					),
				),
			),
	...
	public static function prompt_text( $name, $args = array() ) {
		switch ( $name ) {
			...
			default:
				throw new Exception( 'Unknown prompt: ' . esc_html( $name ) );
		}
	}
			...
			'prompt' => array(
				...
				'messages'    => array(
					array(
						...
						'content' => array(
							'type' => 'text',
							'text' => self::prompt_text( $name, $args ),
						),
					),
				),
			),
